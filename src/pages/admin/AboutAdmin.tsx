import { AboutEditor } from "@/components/admin/AdminDashboard";
...
const AboutAdmin = () => {
  ...
  return (
    <section className="space-y-6 rounded-lg border border-white/10 bg-black/60 p-6 backdrop-blur-xl">
      <div>
        <p className="text-sm text-white/50 mb-1">About Page Management</p>
        <h2 className="text-2xl font-semibold text-white">Customize About Page</h2>
      </div>
      <AboutEditor content={content} setContent={setContent} refreshContent={refreshContent} />
    </section>
  );
};
